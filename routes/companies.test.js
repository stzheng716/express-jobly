...
describe("GET /companies", function () {
  ...
  test("filter by minEmployees and maxEmployees", async function () {
    const resp = await request(app).get("/companies")
    .query({ minEmployees: 500, maxEmployees: 1000 });
    expect(resp.body).toEqual({
      companies:[],
    });
  });

  test("error: filter by minEmployees > maxEmployees", async function () {
    const resp = await request(app).get("/companies").query({ minEmployees: 3, maxEmployees: 1 });
    expect(resp.body).toEqual({
      error: {
        message: "minEmployees can't be greater than maxEmployees",
        status: 400
      }
    });
  });
    ...
    expect(resp.body).toEqual({
      company: {
        ...
        jobs: [
          ...
          {title: 'test job title 2', salary: 2, equity: "0.02", companyHandle: 'c2', id: expect.any(Number)}
        ]
      },
    });
  });
